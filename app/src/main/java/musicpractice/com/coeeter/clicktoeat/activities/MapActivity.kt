...
class MapActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        RestaurantViewModel.getAllRestaurants().observe(this, Observer {
            ...
            if (resultCode == ConnectionResult.SUCCESS) {
                webMap.visibility = View.GONE
                nativeMap.visibility = View.VISIBLE
                    ...
                    gMap.setInfoWindowAdapter(object : GoogleMap.InfoWindowAdapter {
                        ...
                        override fun getInfoWindow(p0: Marker): View? {
                            val restaurantInfoWindowBinding =
                                RestaurantInfoWindowBinding.inflate(layoutInflater)
            ...
            webMap.visibility = View.VISIBLE
            nativeMap.visibility = View.GONE
            val query = restaurant.name.replace(" ", "%20").replace("&", "%zy81")
            webMap.loadUrl(
                "${getString(R.string.base_url)}/restaurants/map?r=$query&lat=${
                    location?.get(
                        "latitude"
                    )
